...
export function LedgerPanel() {
  ...
  const monthlyQuery = useQuery({
    queryKey: ['ledger-monthly', monthlyYear],
    enabled: ledgerView === 'monthly',
    queryFn: async () =>
      (
        await api.get<{ data: LedgerMonthlySummary }>('/api/ledger/monthly', {
  ...
  const displaySummary = ledgerView === 'monthly' ? monthlySummary : summary
  return (
    <div className='space-y-4'>
        ...
        {ledgerView === 'monthly' ? (
          <div className='order-first flex shrink-0 items-center gap-2'>
            ...
            <Select
              value={monthlyYear}
              onValueChange={(value) => {
                if (value !== null) setMonthlyYear(value)
              }}
            >
              <SelectTrigger size='sm' aria-label={t('Select year')}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {Array.from({ length: 11 }, (_, index) => 2026 + index).map(
                  (year) => (
      ...
      <div className='grid gap-3 sm:grid-cols-3'>
        <div className='rounded-lg border p-4'>
          ...
          <p className='mt-1 text-2xl font-semibold'>
            {formatMoney(
              displaySummary?.revenue_cents ?? 0,
              displaySummary?.currency ?? 'USD'
            )}
          </p>
        </div>
        <div className='rounded-lg border p-4'>
          ...
          <p className='mt-1 text-2xl font-semibold'>
            {formatMoney(displaySummary?.expenses ?? 0, 'USD')}
          </p>
        </div>
      ...
      <div className='rounded-lg border'>
        <div className='flex items-center gap-2 border-b px-4 py-3 text-sm'>
          ...
          <button
            type='button'
            aria-current={ledgerView === 'monthly' ? 'page' : undefined}
            className={
              ledgerView === 'monthly'
                ? 'text-foreground font-semibold'
                : 'text-muted-foreground hover:text-foreground'
        ...
        {ledgerView === 'monthly' && (
          <div className='overflow-x-auto px-4 py-4'>
            <div className='text-muted-foreground grid min-w-[32rem] grid-cols-4 gap-3 border-b pb-2 text-xs'>
              <span>{t('Calendar month')}</span>
              <span className='text-right'>{t('Consumption revenue')}</span>
              <span className='text-right'>{t('Recorded expenses')}</span>
              <span className='text-right'>{t('Net profit')}</span>
            </div>
            <div className='min-w-[32rem]'>
              {(
                ...
                <div
                  key={point.month}
                  className='grid grid-cols-4 gap-3 border-b py-3 text-sm last:border-0'
                >
